...
def main():
    ...
    # Display cloud warning if applicable
    if is_cloud:
        st.warning("""
        ### ⚠️ Cloud Deployment Notice
        
        This app is running on Streamlit Cloud, which doesn't support webcam access.
        
        **To use the full functionality with webcam:**
        1. Clone the repository: `git clone https://github.com/chaisthra/gesture-recognition.git`
        2. Install dependencies: `pip install -r requirements.txt`
        3. Run locally: `streamlit run gesture_practice_app.py`
        
        The interface below will still allow you to explore the app's structure and navigation, but webcam functionality will not work.
        """)
    ...
    if app_mode == "Practice Mode":
        # Pass the cloud flag to the practice mode function
        run_practice_mode(gesture_data, is_cloud)
    elif app_mode == "Capture Mode":
        # Pass the cloud flag to the capture mode function
        run_capture_mode(gesture_data, is_cloud)
    elif app_mode == "Manage Gestures":
        run_manage_mode(gesture_data)

def run_practice_mode(gesture_data, is_cloud=False):
    ...
    # Run webcam if active and not in cloud environment
    if st.session_state.practice_active and 'selected_gesture' in locals():
        if is_cloud:
            # Display a message explaining that webcam isn't available in cloud
            webcam_placeholder.markdown("""
            <div style="background-color:rgba(0,0,0,0.7);height:400px;border-radius:10px;display:flex;justify-content:center;align-items:center;color:white;">
                <div style="text-align:center; max-width:80%;">
                    <h3>⚠️ Webcam Not Available in Cloud</h3>
                    <p>Streamlit Cloud doesn't support webcam access. To practice gestures with webcam feedback:</p>
                    <ol style="text-align:left;">
                        <li>Clone the GitHub repository</li>
                        <li>Install dependencies with pip</li>
                        <li>Run the app locally on your computer</li>
                    </ol>
                    <p style="font-size:0.8em;margin-top:20px;">The full experience includes real-time gesture recognition and feedback through your webcam.</p>
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="background-color:rgba(255,255,255,0.1);padding:15px;border-radius:10px;text-align:center;">
                <p>Gesture recognition requires webcam access, which is only available when running locally.</p>
            </div>
            """, unsafe_allow_html=True)
            
            # Update metrics for demonstration only
            ...
            <div class="metric-container">
                ...
                <div class="metric-value">N/A (Cloud)</div>
            </div>
            <div class="metric-container">
...
def run_capture_mode(gesture_data, is_cloud=False):
    ...
    with col2:
        if st.session_state.get('capturing', False):
            if is_cloud:
                # Display cloud message for capture mode
                st.markdown("""
                <div style="background-color:rgba(0,0,0,0.7);height:400px;border-radius:10px;display:flex;justify-content:center;align-items:center;color:white;">
                    <div style="text-align:center; max-width:80%;">
                        <h3>⚠️ Webcam Not Available in Cloud</h3>
                        <p>Capturing new gestures requires webcam access, which is only available when running the app locally.</p>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            else:
                # Original webcam capture code for local execution
